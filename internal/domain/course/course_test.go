	...
	for i := range testCases {
		...
		t.Run(c.Name, func(t *testing.T) {
			...
			for i := 1; i <= extendedCourse.TasksNumber(); i++ {
				...
				extendedDeadline := taskFromExtended.ManualCheckingOptional()
				require.True(t, extendedDeadline.IsZero())
				_, originTestData := taskFromOrigin.AutoCodeCheckingOptional()
				extendedDeadline, extendedTestData := taskFromExtended.AutoCodeCheckingOptional()
				require.True(t, extendedDeadline.IsZero())
				require.Equal(t, originTestData, extendedTestData)
				require.Equal(t, taskFromOrigin.TestingOptional(), taskFromExtended.TestingOptional())
			}
		})
	}
}
